...
import { PassportHero, type Passport, type PassportUser, type ExpProgress } from "@/components/passport-hero";
...
export default async function ProfilePage({ params }: { params: Promise<{ username: string }> }) {
  ...
  const [profile, collection, me] = await Promise.all([
    serverApi<{ user: PassportUser; passport: Passport; follow?: { followers: number; following: number; isFollowing: boolean }; expProgress?: ExpProgress }>(`/users/${encodeURIComponent(username)}`),
  ...
  return (
    <AppShell active="Feeds">
      <div className="mx-auto max-w-[1100px] px-4 pb-12 md:px-6 lg:px-10">
        <PassportHero user={profile.user} passport={profile.passport} isOwn={isOwn} follow={profile.follow} expProgress={profile.expProgress} />
