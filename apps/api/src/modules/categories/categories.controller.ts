...
export class CategoriesController {
  ...
  // Buyer or seller proposes a new sub-category. The admin moderates;
  // approval inserts a real Category row + invalidates the tree cache.
  ...
  async createRequest(
    ...
    // Anti-spam: cap at 5 pending requests per user. Once admin clears
    // them (approve/reject) the user can submit again.
  ...
  // List my own requests (any status). Used on the user's profile or a
  // little "my requests" page so they can see what's pending/approved.
  @Get("requests/mine")
  ...
  // Admin queue. We expose this off /categories for proximity to the
  // tree endpoint; the AdminGuard equivalent is a manual role check.
  @Get("requests")
  async listRequests(
    @CurrentUser() user: SessionUser,
  ...
  async approveRequest(
    ...
    // Tree is cached; bust it so the new category appears immediately.
